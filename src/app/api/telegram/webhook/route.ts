  ...
  try {
    ...
    if (body.callback_query) {
      ...
      if (data.startsWith('deploy_')) {
        ...
        // 🚨 THE ATOMIC LOCK: Delete the draft and return the data at the exact same millisecond.
        // This guarantees that even if Telegram sends 5 duplicate button clicks, only the first one works!
        ...
        if (deleteError) console.error("Draft Delete Error:", deleteError)

        // If length > 0, we successfully got the data before it was deleted.
        if (deletedDrafts && deletedDrafts.length > 0) {
          const draft = deletedDrafts[0]
          
          // 1. Deploy it to the MAIN LIVE FLOOR (Triggers your UI to update!)
          ...
          // 2. Fallback backup to analyses table
          ...
          // 3. Update the Telegram message so the button goes away
          await fetch(`https://api.telegram.org/bot${botToken}/editMessageText`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },

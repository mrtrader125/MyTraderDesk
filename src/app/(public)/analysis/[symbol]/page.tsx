...
import { TrendingUp, TrendingDown, Minus, Users, Unlock, Clock, ArrowRight, Activity } from 'lucide-react'
...
export async function generateMetadata(
  ...
  if (setup) {
    description = `Live ${setup.timeframe} public market structure analysis for ${symbol}. Current desk bias is ${setup.bias || 'Neutral'}. Read the full structural breakdown.`
  }
...
export default async function PublicAnalysisTeaser({ params }: Props) {
  ...
  // 🚨 Fetch ONLY the latest PUBLIC setup for this symbol
  ...
  // 🚨 Fetch other recent PUBLIC setups for internal linking grid
  const { data: rawRecent } = await supabase
    .from('analyses')
    .select('asset_symbol, timeframe, created_at, bias')
    .eq('is_locked', false)
    .neq('asset_symbol', symbol) 
    .order('created_at', { ascending: false })
    .limit(15);
  ...
  for (const item of (rawRecent || [])) {
    if (!seenSymbols.has(item.asset_symbol)) {
      ...
      if (recentSetups.length === 3) break; 
    }
  }

  // If no public setups exist for this symbol, show 404 state
  if (error || !setup) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-white font-sans">
        <h1 className="text-2xl font-black uppercase tracking-widest mb-4">No Public Market Data</h1>
        <p className="text-neutral-500 text-sm font-medium mb-6">There are currently no public analyses available for {symbol}.</p>
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans pt-24 pb-20 px-6 selection:bg-blue-500/30 selection:text-white">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 relative group">
            <div className="bg-[#0a0a0a] border border-neutral-800 rounded-3xl overflow-hidden shadow-2xl relative">
              ...
              <div className="absolute top-4 left-4 z-10 flex gap-2">
                ...
                <span className="bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-lg text-[10px] font-black text-emerald-400 uppercase tracking-widest border border-white/10 flex items-center">
                  <Users size={12} className="mr-1.5" /> Public View
                </span>
              </div>

              {/* Clear Chart Rendering */}
          ...
          {/* Right Column: Public Metadata & Notes */}
          <div className="space-y-6">
            ...
            <div className="bg-[#0a0a0a] border border-neutral-800 rounded-3xl p-6 shadow-xl flex flex-col h-full">
              <h3 className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-4">
                Structural Notes
              </h3>
              
              <div className="text-xs font-medium text-neutral-300 leading-relaxed mb-6 space-y-3 flex-1">
              ...
              <div className="pt-4 border-t border-neutral-800 flex items-center justify-between mt-auto">
                <span className="text-[10px] font-bold text-neutral-500 flex items-center uppercase tracking-widest">
                  <Unlock size={12} className="mr-1.5 text-emerald-500" /> Public Access
                </span>
              </div>
            </div>
          </div>
        ...
        {/* Recent Market Intelligence Grid (Only Public Setups) */}

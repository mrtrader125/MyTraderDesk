  ...
  return (
    <div className="max-w-3xl space-y-8 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between relative z-10 gap-6">
          <div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <h2 className={`text-3xl font-black uppercase tracking-tighter ${userPlan === 'premium' ? 'text-amber-500' : userPlan === 'pro' ? 'text-brand-primary' : userPlan === 'essential' ? 'text-blue-500' : 'text-white'}`}>
                {userPlan === 'premium' ? 'Gold Premium' : `${userPlan} Tier`}
              </h2>
            </div>
            <p className="text-xs font-medium text-neutral-400 mt-2 max-w-md">
              {userPlan === 'premium' ? 'Ultimate clearance granted. Access to Gold Terminal and all scalping modules.' : userPlan === 'pro' ? 'Maximum standard clearance. Full access to all swing markets.' : 'Limited clearance. Upgrade to unlock restricted global markets and bypass time-delays.'}
            </p>
          </div>
          {userPlan !== 'premium' && (
            <button 
      ...
      <div className="border border-red-500/10 rounded-3xl p-8 mt-12">
        ...
        <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-6 leading-relaxed max-w-lg">Canceling your subscription revokes live intelligence access at the end of your billing cycle.</p>

...
function ViewportContent() {
  ...
  if (loading) return (
    <div className="h-screen bg-neutral-100 dark:bg-app-bg flex flex-col items-center justify-center space-y-4">
  ...
  if (!currentSetup) return <div className="h-screen bg-neutral-100 dark:bg-app-bg flex items-center justify-center text-neutral-900 dark:text-white">No data found for {asset}</div>

  const access = getSetupAccess(currentSetup, userPlan)
  const isCurrentBookmarked = watchlist.some(w => w.id === currentSetup.id)

  return (
    <div className="fixed inset-0 bg-neutral-100 dark:bg-app-bg flex overflow-hidden text-neutral-900 dark:text-white select-none touch-none font-sans">
       <div 
       ...
       >
         {access.hasAccess ? (
           ...
           <div className="w-full max-w-4xl aspect-video bg-[#0a0a0a] border border-neutral-200 dark:border-card-border/50 rounded-[2rem] shadow-2xl" style={{ transform: `translate(${pos.x}px, ${pos.y}px) scale(${scale})` }} />
         )}
       </div>

       {!access.hasAccess && (
         <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
           <div className="max-w-sm w-full bg-[#0a0a0a]/90 backdrop-blur-xl border border-neutral-200 dark:border-card-border rounded-3xl p-8 text-center shadow-2xl relative overflow-hidden pointer-events-auto">
             ...
             <div className="w-14 h-14 bg-black border border-neutral-200 dark:border-card-border rounded-2xl flex items-center justify-center mx-auto mb-6 relative z-10 shadow-lg">
             ...
             <div className="flex items-center justify-center space-x-2 bg-black border border-neutral-200 dark:border-card-border rounded-xl py-3 mb-6 relative z-10">
             ...
             <div className="relative z-10 pt-5 border-t border-neutral-200 dark:border-card-border">
...
<button 
  ...
  className="w-10 h-10 bg-[#0a0a0a] border border-neutral-200 dark:border-card-border rounded-xl flex items-center justify-center text-neutral-400 hover:text-neutral-900 dark:text-white hover:border-neutral-600 transition-colors pointer-events-auto shadow-lg shrink-0"
>
  <ArrowLeft size={16} />
</button>           
           <div className="h-10 bg-[#0a0a0a] border border-neutral-200 dark:border-card-border px-4 rounded-xl flex items-center space-x-3 shadow-lg pointer-events-auto">
           ...
           {showInfo && access.hasAccess && (
             <div className="absolute top-12 left-[125px] w-[300px] max-h-[60vh] overflow-y-auto bg-[#0a0a0a]/95 backdrop-blur-xl border border-neutral-200 dark:border-card-border rounded-2xl p-5 shadow-2xl pointer-events-auto animate-in fade-in slide-in-from-top-4">
               ...
               <div className="mt-3 pt-3 border-t border-neutral-200 dark:border-card-border flex items-center space-x-2 text-[9px] font-bold uppercase tracking-widest text-neutral-500">
         ...
         <div className="absolute top-5 left-1/2 -translate-x-1/2 bg-[#0a0a0a] border border-neutral-200 dark:border-card-border p-1.5 rounded-xl shadow-lg pointer-events-auto z-50 flex items-center space-x-1">
            {timeframes.map(t => {
         ...
         <div className={`absolute right-0 top-0 bottom-0 z-40 flex flex-col shadow-2xl transition-all duration-300 border-l border-neutral-200 dark:border-card-border pointer-events-auto ${isSidebarPinned ? 'w-56 bg-[#0a0a0a]/95' : 'w-12 hover:w-56 bg-[#0a0a0a]/80 backdrop-blur-md group/sidebar'}`}>
           <div className={`h-14 flex items-center border-b border-neutral-200 dark:border-card-border transition-all ${isSidebarPinned ? 'justify-between px-4' : 'justify-center group-hover/sidebar:justify-between group-hover/sidebar:px-4'}`}>
...
export default function ViewportPage() {
  return (
    <Suspense fallback={<div className="h-screen bg-neutral-100 dark:bg-app-bg flex items-center justify-center"><Activity className="animate-pulse text-blue-500" size={32} /></div>}>
      <ViewportContent />
    </Suspense>
  )
}

...
import { 
  ...
  Clock, Globe2, CheckCircle2, Lock
} from 'lucide-react'
...
export default function DeskClient() {
  ...
  // --- AUTOMATED RHYTHM TRACKING ---
  const [manualSundayPrep, setManualSundayPrep] = useState(true) // Can toggle manually, defaults to true if setups exist
  const isSundayPrepDone = manualSundayPrep || weeklySetups.length > 0;
  const isTodayFiltered = todaySetups.length > 0;
  const hasExecutedToday = tradesTakenToday > 0 || pendingReconciliation.some(t => t.day === currentDayStr);
  const isWindUpComplete = pendingReconciliation.length === 0;

  // Fake Week Progress for visualization
  const weekProgress = [
    { day: 'M', status: 'perfect' },
    { day: 'T', status: 'missed' },
    { day: 'W', status: 'perfect' },
    { day: 'T', status: hasExecutedToday ? 'perfect' : 'current' }, // Thursday is current in system context
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      const now = new Date();
      setTime(now);
      setCurrentDayStr(now.toLocaleDateString('en-US', { weekday: 'short' }));
      ...
      setSessionInfo({ name: sName, localTime: now.toLocaleTimeString('en-US', { timeZone: tz, hour12: true, hour: '2-digit', minute: '2-digit', second: '2-digit' }) });
  ...
  return (
    <div className="flex h-[calc(100vh-70px)] w-full bg-[#030303] text-zinc-300 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 transition-all duration-300 relative">
        ...
        {/* 🟢 TOP ROW: COMPACT METRICS & OPERATING RHYTHM */}
        <div className="h-28 shrink-0 flex gap-4 p-4 border-b border-zinc-800/60 bg-[#050505]">
          
          {/* Metrics */}
          <div className="flex flex-col gap-2 w-48 shrink-0">
            <div className="flex-1 bg-[#0a0a0a] border border-zinc-800/60 rounded-lg flex flex-col items-center justify-center shadow-sm">
              <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-1 mb-0.5"><Clock size={10}/> Local Time</span>
              <span className="text-sm font-mono text-zinc-100">{mounted && time ? time.toLocaleTimeString('en-US', { hour12: true, hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '--:--:--'}</span>
            </div>
            <div className="flex-1 bg-[#0a0a0a] border border-zinc-800/60 rounded-lg flex flex-col items-center justify-center shadow-sm">
              <span className="text-[9px] font-bold text-blue-400/80 uppercase tracking-widest flex items-center gap-1 mb-0.5"><Globe2 size={10}/> {sessionInfo.name}</span>
              <span className="text-sm font-mono text-white">{sessionInfo.localTime}</span>
            </div>
          </div>

          {/* Operating Rhythm Tracker */}
          <div className="flex-1 bg-[#0a0a0a] border border-zinc-800/60 rounded-lg p-3 shadow-sm flex items-center gap-4">
            
            {/* Week Progress Graph */}
            <div className="w-24 border-r border-zinc-800/50 pr-4 h-full flex flex-col justify-center">
              <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mb-2 flex justify-center">Week Tracker</span>
              <div className="flex justify-between w-full">
                {weekProgress.map((d, i) => (
                  <div key={i} className={`w-3.5 h-3.5 rounded-sm flex items-center justify-center border ${d.status === 'current' ? 'border-blue-500/50 bg-blue-500/20 animate-pulse' : d.status === 'perfect' ? 'border-emerald-500/50 bg-emerald-500/20' : d.status === 'missed' ? 'border-red-500/50 bg-red-500/20' : 'border-zinc-800 bg-zinc-900 text-zinc-600'}`}>
                    {d.status === 'perfect' && <CheckCircle2 size={8} className="text-emerald-400" />}
                    {d.status === 'missed' && <X size={8} className="text-red-400" />}
                    {d.status === 'pending' && <span className="text-[6px] font-bold">{d.day}</span>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex-1 flex justify-between h-full py-1">
              {/* Prep */}
              <div onClick={() => setManualSundayPrep(!manualSundayPrep)} className="flex flex-col items-center justify-center gap-1.5 cursor-pointer group px-2">
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${isSundayPrepDone ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-zinc-950 border-zinc-700 text-transparent'}`}>
                  <CheckCircle2 size={12} />
                </div>
                <span className={`text-[9px] font-bold uppercase tracking-widest text-center ${isSundayPrepDone ? 'text-zinc-400' : 'text-zinc-600 group-hover:text-zinc-400'}`}>Weekly<br/>Prep</span>
              </div>

              {/* Filter */}
              <div className="flex flex-col items-center justify-center gap-1.5 px-2 opacity-90">
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${isTodayFiltered ? 'bg-blue-500/20 border-blue-500 text-blue-400' : 'bg-zinc-950 border-zinc-700 text-transparent'}`}>
                  <CheckCircle2 size={12} />
                </div>
                <span className={`text-[9px] font-bold uppercase tracking-widest text-center ${isTodayFiltered ? 'text-blue-400/80' : 'text-zinc-600'}`}>Daily<br/>Filter</span>
              </div>

              {/* Execution */}
              <div className="flex flex-col items-center justify-center gap-1.5 px-2 opacity-90">
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${hasExecutedToday ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-zinc-950 border-zinc-700 text-transparent'}`}>
                  <CheckCircle2 size={12} />
                </div>
                <span className={`text-[9px] font-bold uppercase tracking-widest text-center ${hasExecutedToday ? 'text-zinc-400' : 'text-zinc-600'}`}>Execution<br/>Logged</span>
              </div>

              {/* Wind up */}
              <div className={`flex flex-col items-center justify-center gap-1.5 px-2 ${!isWeekend ? 'opacity-40 grayscale' : 'opacity-90'}`}>
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${isWeekend && isWindUpComplete && hasExecutedToday ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-zinc-950 border-zinc-700 text-transparent'}`}>
                  {!isWeekend ? <Lock size={10} className="text-zinc-500" /> : <CheckCircle2 size={12} />}
                </div>
                <span className="text-[9px] font-bold uppercase tracking-widest text-center text-zinc-600">Weekend<br/>Windup</span>
              </div>
            </div>

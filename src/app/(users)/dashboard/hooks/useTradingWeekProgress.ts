...
import { WeekProgressDay } from '../types';
...
export function useTradingWeekProgress(
  ...
  const syncLogsLightweight = useCallback(async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const dayLogs = logsData.filter((l: any) => adjustDbToBaseRef.current(l.created_at).toDateString() === dateString);
      ...
      if (dateString === todayString) {
        ...
        if (dayLogs.length > 0) status = dayLogs.some((l: any) => l.execution_type === 'Imperfect') ? 'imperfect' : 'perfect'; 
        else status = 'current';
      } else if (targetDate.getTime() < getBaseDateRef.current().getTime() && dateString !== todayString) {
        isPast = true;
        if (dayLogs.length === 0) status = 'missed'; 
        else status = dayLogs.some((l: any) => l.execution_type === 'Imperfect') ? 'imperfect' : 'perfect';
      }
    ...
    const pending = logsData.filter((l: any) => !l.is_reconciled && l.outcome !== 'HOLD' && adjustDbToBaseRef.current(l.created_at).getTime() >= startOfWeekBase.getTime());
  ...
  return {
    ...
    setPendingReconciliationsCount,
    syncLogsLightweight
  };
}
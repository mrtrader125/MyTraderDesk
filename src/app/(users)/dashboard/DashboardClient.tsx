...
export default function DashboardClient({ 
  ...
  const groupedSetups = useMemo(() => {
    ...
    const baseFiltered = setups.filter(setup => {
      ...
      // RULE 1: ONLY ACTIVE AND WAITING SETUPS
    ...
    Object.values(assetGroups).forEach(group => {
      // Sort the group by latest first
      group.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())

      // Find intraday setups (5m to 4h)
      const intradaySetups = group.filter(s => intradayTfs.includes((s.timeframe || '').toUpperCase().trim()))
      
      if (intradaySetups.length > 0) {
        // RULE 2: Take only the single latest setup in the 5m-4h range
        finalSetups.push(intradaySetups[0]) 
      } else {
        // RULE 3: If no intraday, look for a Daily setup (Ignore Weekly/Monthly)
            ...
            {items.map(setup => {
              ...
              return (
                <div 
                  ...
                  className="bg-[#0a0a0a] border border-neutral-800 hover:border-neutral-600 hover:bg-white/[0.02] transition-all rounded-xl p-3 md:p-2.5 cursor-pointer group flex items-center justify-between shadow-sm overflow-hidden relative"
                >
        ...
        <div className="lg:col-span-8 xl:col-span-9 flex flex-col h-full min-h-0 space-y-3 md:space-y-4">
          ...
          <div className="shrink-0 grid grid-cols-2 md:grid-cols-12 gap-3">
            ...
            <div className="col-span-1 md:col-span-4 xl:col-span-3 bg-[#0a0a0a] border border-neutral-800 p-4 md:p-5 rounded-2xl flex flex-col justify-center relative overflow-hidden shadow-sm">
              ...
              <div className={`text-sm md:text-xl font-black uppercase tracking-widest truncate ${userPlan === 'pro' ? 'text-blue-500' : 'text-neutral-400'}`}>
                {userPlan === 'pro' ? 'Professional' : 'Free Tier'}
              </div>
            </div>
          </div>

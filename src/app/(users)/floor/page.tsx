...
export default function LiveFloorPage() {
  ...
  // Reset Pan/Zoom when modal opens or closes
  useEffect(() => {
    if (expandedImage) {
      setScale(1)
      setPosition({ x: 0, y: 0 })
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    if (zoomIn) {
      newScale = Math.min(scale + zoomFactor, 5) // Max 5x zoom
    } else {
      newScale = Math.max(scale - zoomFactor, 1) // Min 1x zoom
    }
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const { data: postsData } = await supabase.from('terminal_posts').select('*').order('created_at', { ascending: false }).limit(20)
      if (postsData) setPosts(postsData.reverse())

      const { data: squawkData } = await supabase.from('live_squawk').select('*').order('created_at', { ascending: false }).limit(30)
  ...
  const setupRealtime = () => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'terminal_posts' }, (p) => setPosts((c) => c.map(x => x.id === p.new.id ? p.new : x)))
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'terminal_posts' }, (p) => setPosts((c) => c.filter(x => x.id !== p.old.id)))
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 custom-scrollbar bg-[#050505]">
              {posts.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center opacity-50">
                  ...
                  <p className="text-neutral-500 text-[10px] font-bold tracking-wide">The desk is currently analyzing markets.</p>
                </div>
              ) : (
                posts.map((post) => {
                              ...
                              <div className="space-y-5 animate-in fade-in zoom-in-95 duration-300">
                                ...
                                <div className="space-y-4">
                                  <div>
                                    ...
                                    <div className="h-2 w-full bg-[#111] rounded-full overflow-hidden border border-neutral-800"><div className="h-full bg-blue-500 transition-all duration-1000 ease-out" style={{ width: `${results.aligned}%` }}></div></div>
                                  </div>
                                  <div>
                                    ...
                                    <div className="h-2 w-full bg-[#111] rounded-full overflow-hidden border border-neutral-800"><div className="h-full bg-red-500 transition-all duration-1000 ease-out" style={{ width: `${results.counter}%` }}></div></div>
                                  </div>
                                </div>
                              </div>
                            ) : (
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/95 backdrop-blur-md animate-in fade-in duration-200">
          ...
          <div className="absolute top-6 right-6 flex gap-3 z-50">
            <button 
              ...
              className="p-3 bg-neutral-900/80 hover:bg-neutral-800 rounded-full text-neutral-400 hover:text-blue-400 transition-colors border border-neutral-800 hover:border-neutral-600 shadow-2xl"
              title="Reset Zoom"
            >
              <RefreshCw size={20} />
            </button>
            <button 
              ...
              className="p-3 bg-neutral-900/80 hover:bg-neutral-800 rounded-full text-neutral-400 hover:text-red-400 transition-colors border border-neutral-800 hover:border-neutral-600 shadow-2xl"
            >
              <X size={20} />
            </button>
          </div>

...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center space-y-4 text-brand-primary">
        <ShieldAlert className="animate-pulse" size={40} />
        <span className="text-[10px] font-black tracking-[0.3em] uppercase opacity-50">Verifying Clearance...</span>
      </div>
    )
  }
    ...
    <div className="flex h-screen bg-[#050505] text-white font-sans overflow-hidden">
      <div className="w-64 bg-[#0a0a0a] border-r border-neutral-800 flex flex-col z-20 shrink-0">
        ...
        <div className="p-4 border-t border-neutral-800 shrink-0">
          <div className="bg-[#050505] border border-neutral-800 rounded-xl p-3 mb-3 text-brand-primary">
            <span className="text-[8px] font-black uppercase tracking-widest block mb-1 opacity-50">Root Access</span>
            <span className="text-xs font-bold truncate block">{adminEmail}</span>
          </div>
          <button onClick={handleSignOut} className="w-full flex items-center justify-center space-x-2 px-4 py-2.5 rounded-lg text-red-500 hover:bg-red-500/10 text-[10px] font-black uppercase tracking-widest transition-colors">
            <LogOut size={14} />

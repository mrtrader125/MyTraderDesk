...
export default function BroadcastArrayPage() {
  ...
  const [targetTier, setTargetTier] = useState<'ALL' | 'PRO' | 'FREE'>('ALL')
  ...
  async function fetchHistory() {
    try {
      // NOTE: Ensure you have a 'notifications' table in Supabase!
  ...
  const handleTransmit = async () => {
    ...
    try {
      ...
      console.error(err)
      alert("Transmission failed. Make sure you have created the 'notifications' table in Supabase.")
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-[#0a0a0a] border border-neutral-800 rounded-[2rem] p-6 sm:p-8 shadow-xl relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-[#050505] border border-neutral-800 rounded-xl py-3 px-4 text-xs font-bold text-white outline-none focus:border-brand-primary/50 transition-colors appearance-none"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    value={urgency} onChange={(e: any) => setUrgency(e.target.value)}
                    className="w-full bg-[#050505] border border-neutral-800 rounded-xl py-3 px-4 text-xs font-bold text-white outline-none focus:border-brand-primary/50 transition-colors appearance-none"
                  >
              ...
              <div className="space-y-2 pt-2 border-t border-neutral-800/50">
                <label className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">Target Network</label>
                <div className="flex gap-2">
                  {['ALL', 'PRO', 'FREE'].map((tier) => (
                    <button
                      key={tier}
                      onClick={() => setTargetTier(tier as any)}
                      className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${targetTier === tier ? 'bg-white text-black border-white shadow-md' : 'bg-[#050505] text-neutral-500 border-neutral-800 hover:border-neutral-600'}`}
                    >
                      {tier} Network
                    </button>
                  ))}
              ...
              <div className="space-y-4 pt-2 border-t border-neutral-800/50">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    placeholder="e.g., High-Impact News Approaching"
                    className="w-full bg-[#050505] border border-neutral-800 rounded-xl py-3 px-4 text-sm font-black text-white outline-none focus:border-brand-primary/50 transition-colors"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    placeholder="Detail the instructions or alert for the operators..."
                    className="w-full h-24 bg-[#050505] border border-neutral-800 rounded-xl py-3 px-4 text-xs font-medium text-white outline-none focus:border-brand-primary/50 transition-colors resize-none"
                  />
                </div>
              </div>
              ...
              <button 
                ...
                disabled={transmitting || !title || !message}
                className="w-full py-4 bg-brand-primary text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-brand-primary/90 transition-all shadow-[0_0_20px_rgba(var(--brand-primary-rgb),0.3)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
              >
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-[#050505] border border-neutral-800 border-dashed rounded-[2rem] p-6">
            <h3 className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-6 flex items-center justify-center">
              <Globe size={14} className="mr-2 text-neutral-600" /> Live Operator Preview
            </h3>

            <div className="flex items-center justify-center min-h-[150px]">
              {type === 'BROADCAST' ? (
                // Broadcast Dashboard Preview
                <div className={`w-full p-4 rounded-xl relative overflow-hidden border ${urgency === 'CRITICAL' ? 'bg-red-500/10 border-red-500/20' : urgency === 'WARNING' ? 'bg-amber-500/10 border-amber-500/20' : 'bg-blue-500/10 border-blue-500/20'}`}>
                  ...
                  <span className={`text-[9px] font-black uppercase tracking-widest block mb-1.5 relative z-10 ${urgency === 'CRITICAL' ? 'text-red-400' : urgency === 'WARNING' ? 'text-amber-400' : 'text-blue-400'}`}>
                    {title || 'System Broadcast'}
                  </span>
                  <p className={`text-xs leading-relaxed font-medium relative z-10 ${urgency === 'CRITICAL' ? 'text-red-100' : urgency === 'WARNING' ? 'text-amber-100' : 'text-blue-100'}`}>
                    {message || 'Your message payload will appear here.'}
                  </p>
                </div>
              ) : (
                // Bell Dropdown Alert Preview
          ...
          <div className="bg-[#0a0a0a] border border-neutral-800 rounded-[2rem] p-6 shadow-xl flex flex-col h-[400px]">
            <h3 className="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center shrink-0">
              <Clock size={16} className="mr-2 text-neutral-500" /> Transmission History
            </h3>
                  ...
                  <div key={item.id} className="bg-[#050505] border border-neutral-800 rounded-xl p-4 flex items-start justify-between group">
                    <div className="flex-1 min-w-0 pr-4">
                      <div className="flex items-center space-x-2 mb-1">
                        <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${item.type === 'BROADCAST' ? 'bg-brand-primary/20 text-brand-primary' : 'bg-neutral-800 text-neutral-400'}`}>
                          {item.type}
                        </span>
                    ...
                    <button 
                      ...
                      className="p-1.5 text-neutral-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors shrink-0"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>

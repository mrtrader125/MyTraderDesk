...
export default function AdminDashboard() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-8 pb-12 animate-in fade-in duration-500">
      <div>
        <h2 className="text-3xl font-black text-neutral-900 dark:text-white uppercase tracking-tighter italic">Platform <span className="text-brand-primary">Analytics</span></h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 items-stretch">
        ...
        <StatCard label="Total Traders" value={data.totalUsers} subValue={`${data.paidUsers} Paid Seats`} icon={Users} color="text-neutral-900 dark:text-white" />
                ...
                {data.topSearches.map((item: any, i: number) => (
                  <div key={i} className="flex flex-col">
                    <div className="flex justify-between items-end mb-1">
                      <span className="text-xs font-bold text-neutral-900 dark:text-white tracking-widest">{item.term}</span>
            ...
            <div className="flex-1 overflow-y-auto max-h-[250px] pr-2 space-y-2 scrollbar-hide">
              {data.recentActivity.length > 0 ? (
                data.recentActivity.map((log: any) => {
                  ...
                  if (log.action === 'SEARCH') { actionColor = "text-neutral-900 dark:text-white"; actionText = "Searched for"; Icon = Search }
                    ...
                    <div key={log.id} className="flex items-center justify-between p-3 rounded-xl bg-neutral-50 dark:bg-[#050505] border border-neutral-200 dark:border-neutral-800/50">
                      <div className="flex items-center space-x-3 overflow-hidden">
                        ...
                        <div className="flex flex-col min-w-0 pr-2">
                          <span className="text-[10px] font-black text-neutral-900 dark:text-white truncate">{log.user}</span>
...
function PlanRow({ label, count, color, icon: Icon }: any) {
  return (
    <div className="flex items-center justify-between p-4 rounded-2xl bg-neutral-50 dark:bg-[#050505] border border-neutral-200 dark:border-neutral-800">
      ...
      <span className="text-sm font-black text-neutral-900 dark:text-white">{count} <span className="text-[9px] text-neutral-600 uppercase">Users</span></span>
    </div>
  )
}

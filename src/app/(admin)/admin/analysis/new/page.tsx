...
export default function PublishSetupPage() {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from('queued_analyses').insert([{
        ...
        bias: formData.bias,
        status: formData.status, // <-- Pushing status to database
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-6 animate-in fade-in duration-500 bg-zinc-950 min-h-screen p-4 md:p-6">
      
      <button onClick={() => router.push('/admin/analysis')} className="flex items-center text-sm font-medium text-zinc-400 hover:text-zinc-100 transition-colors">
      ...
      <div className="bg-zinc-900/50 border border-zinc-800/50 rounded-2xl p-6 lg:p-8 shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-6 flex flex-col justify-between">
            ...
            <div className="space-y-2 pt-4 border-t border-zinc-800/50">
              <label className="text-sm font-medium text-zinc-400">Analysis Notes</label>
              <textarea value={formData.content} onChange={(e) => setFormData({...formData, content: e.target.value})} placeholder="Setup logic, entry triggers..." className="w-full h-24 bg-zinc-950 border border-zinc-800 rounded-lg py-3 px-3 text-sm text-zinc-200 outline-none focus:border-zinc-600 transition-colors resize-none placeholder:text-zinc-600 leading-relaxed" />
            </div>

            <div className="p-4 bg-zinc-950 rounded-xl border border-zinc-800/50 flex items-center justify-between mt-2">
              <div>
                <h4 className="text-sm font-medium text-zinc-200 flex items-center">
                  <Star className={`w-4 h-4 mr-2 ${formData.is_featured ? 'text-amber-400 fill-amber-400/20' : 'text-zinc-500'}`} />
              ...
              <button
                ...
                className={`w-11 h-6 rounded-full transition-colors relative flex items-center shrink-0 ${formData.is_featured ? 'bg-amber-500/20 border border-amber-500/30' : 'bg-zinc-800 border border-zinc-700'}`}
              >
                <div className={`w-4 h-4 rounded-full transition-all absolute ${formData.is_featured ? 'bg-amber-400 right-1' : 'bg-zinc-400 left-1'}`}></div>
              </button>
            </div>
          ...
          <div className="flex flex-col h-full min-h-[400px]">
            <label className="text-sm font-medium text-zinc-400 mb-2 block">Chart Image Capture</label>
            ...
            <div className="mt-6 flex justify-end shrink-0">
              <button 
                ...
                className="w-full flex justify-center items-center px-6 py-3.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
              >

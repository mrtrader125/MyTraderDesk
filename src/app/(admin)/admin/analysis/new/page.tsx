    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div>
        <h2 className="text-3xl font-black text-white uppercase tracking-tighter italic">
          Deploy <span className="text-brand-primary">Intelligence</span>
        </h2>
      ...
      <div className="bg-[#0a0a0a] border border-neutral-800 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 pt-2 border-t border-neutral-800/50">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button 
                  onClick={() => setFormData({...formData, bias: 'BULLISH'})}
                  className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${formData.bias === 'BULLISH' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-[#050505] border-neutral-800 text-neutral-500 hover:border-neutral-600'}`}
                >
                  <TrendingUp size={16} className="mb-1" />
                  <span className="text-[9px] font-black uppercase tracking-widest">Bullish</span>
                </button>
                <button 
                  onClick={() => setFormData({...formData, bias: 'NEUTRAL'})}
                  className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${formData.bias === 'NEUTRAL' ? 'bg-neutral-800 border-neutral-500 text-white shadow-[0_0_15px_rgba(255,255,255,0.1)]' : 'bg-[#050505] border-neutral-800 text-neutral-500 hover:border-neutral-600'}`}
                >
                  <Minus size={16} className="mb-1" />
                  <span className="text-[9px] font-black uppercase tracking-widest">Neutral</span>
                </button>
                <button 
                  onClick={() => setFormData({...formData, bias: 'BEARISH'})}
                  className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${formData.bias === 'BEARISH' ? 'bg-red-500/10 border-red-500 text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'bg-[#050505] border-neutral-800 text-neutral-500 hover:border-neutral-600'}`}
                >
            ...
            <div className="space-y-2 pt-2 border-t border-neutral-800/50">
              ...
              <textarea 
                ...
                placeholder="Detail the setup logic, entry triggers, or invalidation levels..."
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 min-h-[300px] relative">
              ...
              <div className={`absolute inset-0 border-2 border-dashed rounded-[1.5rem] flex flex-col items-center justify-center transition-all duration-300 overflow-hidden ${previewUrl ? 'border-brand-primary/50 bg-black' : 'border-neutral-800 bg-[#050505] hover:border-neutral-600 hover:bg-neutral-900/50'}`}>
                ...
                {previewUrl && (
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity z-10 pointer-events-none">
                    <span className="text-[10px] font-black text-white uppercase tracking-widest bg-black/60 px-4 py-2 rounded-lg backdrop-blur-md">Change Image</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mt-8 pt-6 border-t border-neutral-800 flex justify-end">
          <button 
            ...
            className="flex items-center px-10 py-4 bg-brand-primary text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-brand-primary/90 transition-all shadow-[0_0_30px_rgba(var(--brand-primary-rgb),0.4)] disabled:opacity-50 disabled:cursor-not-allowed group"
          >
            {loading ? (
              <Activity className="animate-spin mr-3" size={18} />
            ) : (
              <Rocket className="mr-3 group-hover:-translate-y-1 group-hover:translate-x-1 transition-transform" size={18} />
            )}
